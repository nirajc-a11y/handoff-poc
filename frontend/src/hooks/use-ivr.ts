...
export function useIVRMenus() {
  ...
  return useQuery<IVRMenu[]>({ queryKey: ['ivr-menus'], queryFn: () => api.get('/ivr/menus'), enabled: !!tenantId })
}

export function useIVRMenu(id: string | null) {
  return useQuery<IVRMenu>({ queryKey: ['ivr-menus', id], queryFn: () => api.get(`/ivr/menus/${id}`), enabled: !!id })
}
